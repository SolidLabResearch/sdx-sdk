...
export class MutationHandler {
    ...
    private async handleCreateMutation<TArgs>(source: IntermediateResult, args: TArgs, context: SolidLDPContext, info: GraphQLResolveInfo, rootTypes: string[]): Promise<IntermediateResult> {
        ...
        source.quads = this.generateTriplesForInput(source.subject, input, utils.unwrapNonNull(inputType) as GraphQLInputObjectType, namedNode(className));
    ...
    private async handleUpdateMutation<TArgs>(source: IntermediateResult, args: TArgs, context: SolidLDPContext, info: GraphQLResolveInfo): Promise<IntermediateResult> {
        const returnType = info.schema.getType(utils.unwrapNonNull(info.returnType).toString()) as GraphQLObjectType;
        const targetUrl = await context.resolver.resolve(source.parentClassIri!, new TargetResolverContext(this.ldpClient))
        const input = (args as any).input;
    ...
    private getNewInstanceID(input: Record<string, any>, resourceType: ResourceType): string {
        switch (resourceType) {
    ...
    private generateTriplesForInput(subject: NamedNode, input: Record<string, any>, inputDefinition: GraphQLInputObjectType, classUri: NamedNode): Quad[] {
        const quads: Quad[] = [];
    ...
    private generateTriplesForUpdate(source: Quad[], input: Record<string, any>, subject: NamedNode, objectTypeDefinition: GraphQLObjectType): { inserts: Quad[], deletes: Quad[] } {
