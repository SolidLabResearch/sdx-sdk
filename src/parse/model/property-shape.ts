...
export class PropertyShape {
  ...
  constructor(public quads: Quad[], private context: Context) {
    ...
    this.name =
      this.parseObject(store, vocab.SHACL.name) ??
      utils.parseNameFromUri(this.parseObject(store, vocab.SHACL.path)!);
  ...
  private findMatchingShapeType(clazz?: string): string | undefined {
    ...
    const match = this.context
      .getStore()
      .getQuads(null, vocab.SHACL.targetClass, namedNode(clazz), null);
