...
var _ldpclientjs = require('./commons/ldp/ldp-client.js');
var _typesjs = require('./types.js');
var _utiljs = require('./util.js');
var _vocabjs = require('./vocab.js');
var _targetresolversjs = require('./backends/ldp/target-resolvers.js');
const { namedNode, quad, literal } = _n3.DataFactory;
const ID_FIELD = "id";
const SLUG_FIELD = "slug";
function fieldResolver(ldpClient) {
  ...
  async function handleQuery(source, args, context, info, rootTypes) {
    ...
    if (rootTypes.includes(parentType.name)) {
      ...
      const targetUrl = await context.resolver.resolve(className, new (0, _targetresolversjs.TargetResolverContext)(ldpClient));
  ...
  async function handleCreateMutation(source, args, context, info, rootTypes) {
    ...
    source.quads = generateTriplesForInput(source.subject, input, _utiljs.unwrapNonNull.call(void 0, inputType), namedNode(className));
    switch (source.resourceType) {
      case _typesjs.ResourceType.DOCUMENT:
        await new (0, _ldpclientjs.LdpClient)().patchDocument(targetUrl.toString(), source.quads);
    }
    return source;
  }
  ...
  async function handleDeleteMutation(source, args, context, info) {
    ...
    const targetUrl = await context.resolver.resolve(source.parentClassIri, new (0, _targetresolversjs.TargetResolverContext)(ldpClient));
    switch (source.resourceType) {
      case _typesjs.ResourceType.DOCUMENT:
        await new (0, _ldpclientjs.LdpClient)().patchDocument(targetUrl.toString(), null, source.quads);
    }
    return source;
  }
  async function handleUpdateMutation(source, args, context, info) {
    ...
    switch (source.resourceType) {
      case _typesjs.ResourceType.DOCUMENT:
        await new (0, _ldpclientjs.LdpClient)().patchDocument(targetUrl.toString(), inserts, deletes);
    }
    const store = new (0, _n3.Store)(source.quads);
