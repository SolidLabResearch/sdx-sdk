...
function fieldResolver(location) {
  ...
  async function handleCreateMutation(source, args, context, info, rootTypes) {
    ...
    const resourceType = _typesjs.ResourceType.DOCUMENT;
    ...
    const content = generateTriplesForInput(namedNode(id), input, _utiljs.unwrapNonNull.call(void 0, inputType), namedNode(classUri));
    switch (resourceType) {
      case _typesjs.ResourceType.DOCUMENT:
        await new (0, _ldpclientjs.LdpClient)().patchDocument(targetUrl, content);
    }
    return content;
  }
  ...
  async function handleDeleteMutation(source, args, context, info) {
    ...
    switch (resourceType) {
      case _typesjs.ResourceType.DOCUMENT:
        await new (0, _ldpclientjs.LdpClient)().patchDocument(targetUrl, null, source);
    }
    return source;
  }
}
