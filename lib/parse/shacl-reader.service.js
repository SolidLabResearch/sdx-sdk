...
let ShaclReaderService = class {
  ...
  */
  ...
  generateInputObjectType(type, name, mutationType, context) {
    ...
    if (!inputType) {
      let fields = Object.fromEntries(Object.entries(type.getFields()).filter(([, field]) => _commons.utils.isOrContainsScalar(field.type)).filter(([, field]) => !isIdentifier(field)).map(([name2, field]) => [name2, toInputField(field, mutationType)]));
