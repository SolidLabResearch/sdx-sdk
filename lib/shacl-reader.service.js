  ...
  generateEntryPoints(types) {
    const query = new (0, _graphql.GraphQLObjectType)({
      ...
      fields: types.reduce((prev, type) => ({
        ...prev,
        // Singular type
        [_utiljs.decapitalize.call(void 0, type.name)]: {
          type,
          args: { id: { type: _graphql.GraphQLString } }
        },
        // Multiple types
        [_utiljs.plural.call(void 0, _utiljs.decapitalize.call(void 0, type.name))]: {
  ...
  */
  generateMutationEntryPoints(context) {
    ...
    const mutation = new (0, _graphql.GraphQLObjectType)({
      ...
      fields: types.reduce((prev, type) => {
        const createName = `create${_utiljs.capitalize.call(void 0, type.name)}`;
        const mutateName = `mutate${_utiljs.capitalize.call(void 0, type.name)}`;
        return {
          ...prev,
          // create type
          [createName]: {
            type: new (0, _graphql.GraphQLNonNull)(type),
            args: { input: { type: new (0, _graphql.GraphQLNonNull)(this.generateInputObjectType(type, `${_utiljs.capitalize.call(void 0, createName)}Input`, "create", context)) } }
          },
          // edit types
          [mutateName]: {
  ...
  generateInputObjectType(type, name, mutationType, context) {
    ...
    if (!inputType) {
      let fields = Object.fromEntries(Object.entries(type.getFields()).filter(([_, field]) => _utiljs.isOrContainsScalar.call(void 0, field.type)).filter(([_, field]) => !isIdentifier(field)).map(([name2, field]) => [name2, toInputField(field, mutationType)]));
  ...
  generateMutationObjectType(type, context) {
    return new (0, _graphql.GraphQLObjectType)({
      name: `${_utiljs.capitalize.call(void 0, type.name)}Mutation`,
  ...
  generateMutationObjectTypeFields(type, context) {
    ...
    const inputType = this.generateInputObjectType(type, `Update${_utiljs.capitalize.call(void 0, type.name)}Input`, "update", context);
    ...
    const extra = Object.values(type.getFields()).reduce((acc, field) => {
      if (_utiljs.isOrContainsObjectType.call(void 0, field.type)) {
  ...
  generateMutationObjectTypeFieldsForCollection(field, parentType, context) {
    const addName = `add${_utiljs.capitalize.call(void 0, field.name)}`;
    const removeName = `remove${_utiljs.capitalize.call(void 0, field.name)}`;
    const linkName = `link${_utiljs.capitalize.call(void 0, field.name)}`;
    const unlinkName = `unlink${_utiljs.capitalize.call(void 0, field.name)}`;
    const returnType = new (0, _graphql.GraphQLNonNull)(parentType);
    const fieldType = _utiljs.toActualType.call(void 0, field.type);
    return {
      [addName]: {
        type: returnType,
        args: {
          input: {
            type: new (0, _graphql.GraphQLNonNull)(this.generateInputObjectType(fieldType, `Create${_utiljs.capitalize.call(void 0, fieldType.name)}Input`, "create", context))
          }
        }
      },
      [removeName]: {
  ...
  generateMutationObjectTypeFieldsForSingular(field, parentType, context) {
    const setName = `set${_utiljs.capitalize.call(void 0, field.name)}`;
    const clearName = `clear${_utiljs.capitalize.call(void 0, field.name)}`;
    const returnType = new (0, _graphql.GraphQLNonNull)(parentType);
    const fieldType = _utiljs.toActualType.call(void 0, field.type);
    return {
      [setName]: {
