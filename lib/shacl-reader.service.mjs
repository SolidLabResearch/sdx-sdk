...
let ShaclReaderService = class {
  ...
  */
  ...
  generateInputObjectType(type, name, mutationType, context) {
    ...
    if (!inputType) {
      let fields = Object.fromEntries(Object.entries(type.getFields()).filter(([_, field]) => isOrContainsScalar(field.type)).filter(([_, field]) => !isIdentifier(field)).map(([name2, field]) => [name2, toInputField(field, mutationType)]));
  ...
  generateMutationObjectTypeFieldsForCollection(field, parentType, context) {
    ...
    const fieldType = toActualType(field.type);
    return {
      [addName]: {
        type: returnType,
        args: {
