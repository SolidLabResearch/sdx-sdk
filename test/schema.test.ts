...
import {
  ...
  decapitalize,
  getDirectivesMap
} from '../src/commons/util';

const TEST_ASSETS_SCHEMA_FILE = './test/assets/gql/schema.graphqls';

describe('GQL Schema is read', () => {
  const schemaPromise = readFile(TEST_ASSETS_SCHEMA_FILE).then((file) =>
    makeExecutableSchema({
      typeDefs: file.toString()
    })
  );
  ...
  it('with directive definitions', async () => {
    ...
    // 3 custom directives + 4 standard directives (include, skip, deprecated, specifiedBy)
    expect(directives.length).toBe(3 + 4);
  ...
  it('with applied directives', async () => {
    const schema = await schemaPromise;
  ...
  it('with Query Types', async () => {
    ...
    const checkQueryType = (type: GraphQLObjectType) => {
      ...
      expect(idField.astNode!.directives).toIncludeAllPartialMembers([
        {
          kind: Kind.DIRECTIVE,
          arguments: []
        }
      ]);
      expect(idField.astNode!.directives![0]!.name.value).toEqual('identifier');
      expect(fields.filter((fields) => fields.name !== 'id')).toSatisfyAll(
        (field: GraphQLField<any, any>) => {
          const directives = getDirectivesMap(field);
          return (
            ...
            directives.property?.iri.startsWith('http://schema.org/')
          );
        }
      );
    };
  ...
  describe('with Mutation Types', () => {
    ...
    const checkMutationType = (type: GraphQLObjectType) => {
      ...
      expect(fields).toSatisfyAll(
        (field: GraphQLField<any, any>) =>
          field.name.slice(0, 1).toLocaleLowerCase() === field.name.slice(0, 1)
      );
    };
    ...
    it('with correctly paired link fields', async () => {
      types.forEach((type) => {
        ...
        checkIfFieldsAreLinkPaired([setFields, clearFields], ['set', 'clear']);
      });
    });
  });
});
